...
test("LOT4A pull: seul le snapshot académique reçoit les versions de notes", () => {
  assert.match(pullRoute, /attachRelayStudentGradeVersions/);
  assert.match(
    pullRoute,
    /const snapshot = academicChanged\s*\? await attachRelayStudentGradeVersions\(service, institutionId, rawSnapshot\)\s*:\s*rawSnapshot;/s,
  );
});
