...
helper_anchor = '''function makeFullNameKey(fullName: string) {
  return stripAccents(normSpaces(fullName)).toLowerCase();
}
'''
helper_insert = '''function makeFullNameKey(fullName: string) {
  return stripAccents(normSpaces(fullName)).toLowerCase();
}
...
'''
if "function makeLooseIdentityKey(fullName: string)" not in route:
    if helper_anchor not in route:
        raise SystemExit("helper anchor not found")
    route = route.replace(helper_anchor, helper_insert, 1)

loose_anchor = "  type StudentResolution =\n"
loose_block = '''  // Index d'identité tolérant uniquement les différences de forme
...
'''
if "const existingByLooseNameKey" not in route:
    if loose_anchor not in route:
        raise SystemExit("student resolution anchor not found")
    route = route.replace(loose_anchor, loose_block + loose_anchor, 1)

resolver_start = route.find(
    "  function resolveExistingStudent(row: ParsedStudentRow): StudentResolution {"
)
resolver_end = route.find("\n  const inputMatriculesByName", resolver_start)
if resolver_start < 0 or resolver_end < 0:
    raise SystemExit("resolver boundaries not found")

resolver = '''  function resolveExistingStudent(row: ParsedStudentRow): StudentResolution {
...
'''
route = route[:resolver_start] + resolver + route[resolver_end:]
...
test_marker = 'test("l\'import bloque un matricule déjà attribué à une autre identité"'
