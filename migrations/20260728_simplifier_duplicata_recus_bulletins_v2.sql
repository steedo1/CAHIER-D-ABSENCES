-- Mon Cahier - Simplification professionnelle des duplicatas
-- Reçus et bulletins : l'utilisateur autorisé décide de tirer le duplicata.
-- Aucun motif ni statut bloquant. Le document conserve son numéro original
-- et chaque duplicata reste numéroté dans l'historique.

BEGIN;

-- Les anciens statuts techniques ne doivent plus empêcher un duplicata.
UPDATE public.official_document_issues
SET status = 'valid',
    revoked_at = NULL,
    revoked_by = NULL,
    revoke_reason = NULL,
    updated_at = now()
WHERE document_type IN ('receipt', 'bulletin')
  AND status <> 'valid';
