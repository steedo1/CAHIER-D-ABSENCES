...
export class RelayStore {
  ...
  status(): RelayStatus {
    ...
    const cursorErrorState = (
      stream: "cloud_push" | "cloud_pull",
      institutionId?: string,
    ) => {
      const row = institutionId
        ? this.db.prepare(`
            ...
            FROM sync_cursors
            WHERE institution_id = ? AND stream = ?
          `).get(institutionId, stream)
        : this.db.prepare(`
            SELECT last_error_at, last_error
            FROM sync_cursors
            WHERE stream = ? AND last_error_at IS NOT NULL
    ...
    const cloudPushState = (institutionId?: string) => {
      const value = cursorErrorState("cloud_push", institutionId);
      return {
        last_cloud_sync_error_at: value?.last_error_at || null,
        last_cloud_sync_error: value?.last_error || null,
      };
    };
    const cloudPullState = (institutionId?: string) => {
      const value = cursorErrorState("cloud_pull", institutionId);
      return {
        last_cloud_pull_error_at: value?.last_error_at || null,
        last_cloud_pull_error: value?.last_error || null,
      };
    };
    const scheduleRevision = (institutionId: string) => {
      const raw = getInstitutionMeta(
        this.db,
        institutionId,
    ...
    return {
      ...
      ...cloudPushState(),
      last_cloud_pull_at: latestInstitutionMeta(this.db, "last_cloud_pull_at"),
      ...cloudPullState(),
