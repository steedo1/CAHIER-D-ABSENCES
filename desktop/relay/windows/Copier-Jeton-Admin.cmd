...
set "RELAY_TOKEN="
for /f "usebackq delims=" %%T in (`"%RELAY_NODE%" dist\src\cli.mjs access --token-only 2^>nul`) do set "RELAY_TOKEN=%%T"
if not defined RELAY_TOKEN (
  set /p "RELAY_SCHOOL_CODE=Code unique de l'etablissement : "
  for /f "usebackq delims=" %%T in (`"%RELAY_NODE%" dist\src\cli.mjs access --token-only --institution-code "!RELAY_SCHOOL_CODE!" 2^>nul`) do set "RELAY_TOKEN=%%T"
)
if not defined RELAY_TOKEN (
