...
export default function MontageProjectPreview({
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Impossible de charger l’aperçu du brouillon."
      );
    } finally {
      setLoading(false);
    }
  ...
  const groups =
    mode === "class"
      ? groupBy(assignments, "class_label")
      : groupBy(assignments, "teacher_name");
          ...
          <div className="relative p-6 sm:p-8">
            ...
            <div className="relative">
              ...
              <h1 className="mt-5 text-3xl font-black tracking-tight text-white sm:text-4xl">
                {project?.name || "Pré-montage emploi du temps"}
              </h1>

              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">
                Consultation du résultat généré en brouillon. Cette page ne publie rien
                dans les emplois du temps officiels.
              </p>
        ...
        {!loading && project && (
          <>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <StatBox
                label="Statut moteur"
                value={result?.status === "generated_preview" ? "Généré" : "En attente"}
              />
            </div>
            ...
            {assignments.length === 0 ? (
              <div className="rounded-[28px] border border-amber-200 bg-amber-50 p-6 text-amber-950 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-black">Aucun pré-montage généré</p>
                    <p className="mt-1 text-sm">
                      Retourne sur la page Montage emploi du temps, puis clique sur
                      “Générer le pré-montage” pour ce brouillon.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="text-xl font-black">Aperçu du pré-montage</h2>
                    <p className="mt-1 text-sm text-slate-500">
                      Les cours sont affichés en lecture seule, sans publication officielle.
                    </p>
                  </div>
                    ...
                    >
                      <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">
                        ...
                        <p className="text-xs font-semibold text-slate-500">
                          {group.items.length} cours placé
            ...
            {(unplaced.length > 0 || diagnostics.length > 0) && (
              <div className="rounded-[28px] border border-amber-200 bg-amber-50 p-6 text-amber-950 shadow-sm">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" />
                  <div>
                    <h2 className="font-black">Alertes du pré-montage</h2>
                    ...
                    <div className="mt-3 space-y-2 text-sm">
                      ...
                      {unplaced.map((item, index) => (
                        <p key={`unplaced-${index}`}>
                          • Non placé : {item.class_label || "Classe"} —{" "}
                          {item.subject_label || "Matière"} —{" "}
                          {item.teacher_name || "Enseignant"}
                        </p>
                      ))}
                    </div>
                  </div>
