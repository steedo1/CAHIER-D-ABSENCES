...
function normalizeItem(raw: any, progressionId: string, institutionId: string | null, index: number) {
...
async function getNationalProgression(srv: any, id: string) {
  const { data, error } = await srv
    .from("textbook_progression_templates")
    .select("id,institution_id,scope")
    .eq("id", id)
    .eq("scope", "national")
    .maybeSingle();
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  const { id } = await context.params;
  const auth = await requireNationalTextbookManager();
  if (!auth.ok) return auth.response;
  const { srv, userId } = auth.ctx;
  ...
  const nationalInstitutionId = (progression as any).institution_id ? String((progression as any).institution_id) : null;
  const items = sourceItems
    .map((raw: any, index: number) => normalizeItem(raw, id, nationalInstitutionId, index))
    .filter(Boolean);
