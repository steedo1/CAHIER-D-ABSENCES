...
/** D�tection du s�parateur + guillemets */
...
function parseCsvStudentsFlexible(raw: string) {
  ...
  // Expressions ANCR�0ES pour �viter "no" �x "nom"
  const idx = {
    numero:   H.findIndex(h => /^(n�|n�|no|numero|num|#)$/i.test(hCompact(h))),
...
export async function POST(req: Request) {
  ...
  if (action === "commit") {
    ...
    // Inscriptions : respecter NOT NULL sur start_date (et institution_id si pr�sent)
    const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
    const enrollRows = (created || []).map(s => ({
      class_id,
      student_id: s.id,
      institution_id: inst, // si la colonne existe (ne g�ne pas si elle y est)
    ...
    if (enrollRows.length) {
      const { error: e2 } = await srv
        ...
        // n�cessite une contrainte UNIQUE (class_id, student_id)
