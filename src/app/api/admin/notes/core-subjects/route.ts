...
function selectTopSubjectsWithTies(
  ...
  for (const row of rows) {
    ...
    const subject_name =
      String(row.subject_name || "Discipline").trim() || "Discipline";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const subjectIds = coeffBase.map((r) => r.subject_id);
    ...
    const allEligibleItems: SubjectItem[] = coeffBase.map((r) => ({
    ...
    const coeffThreshold =
      items.length >= 4
        ? items[3]?.coeff ?? null
        : items[items.length - 1]?.coeff ?? null;
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        level_subject_count: allEligibleItems.length,
      },
      items,
    });
  } catch (e: any) {
