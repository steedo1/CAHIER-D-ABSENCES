...
function buildPhoneVariants(raw: string): PhoneVariants {
  ...
  const variants = uniq<string>([
    t, t.replace(/\s+/g, ""), digits,
    `+${digits}`, `+${cc}${local10}`, `+${cc}${localNo0}`,
...
export async function POST(req: Request) {
  try {
    ...
    const subject_id = b?.subject_id && String(b.subject_id).trim()
      ? String(b.subject_id).trim()
      : null;
    ...
    // Durée obligatoire (arrondie, min 1)
    ...
    // Horodatage de début (utilisé comme créneau) — on conserve la valeur UI si fournie
    ...
    // Heure réelle du clic (serveur) — indépendante de l'heure du téléphone
    ...
    if (uniqTeachers.length === 0) return NextResponse.json({ error: "no_teacher_for_subject" }, { status: 400 });
    if (uniqTeachers.length > 1)   return NextResponse.json({ error: "ambiguous_teacher_for_subject" }, { status: 400 });
    ...
    return NextResponse.json({
      item: {
        ...
        subject_name: (subjName as any)?.custom_name ?? (subjName as any)?.subjects?.name ?? null,
