...
export default function AnnualMatrixPage() {
  ...
  const stats = useMemo(() => {
    ...
    if (!valid.length) return { count: matrixRows.length, classAvg: null, highest: null, lowest: null };
  ...
  useEffect(() => {
    ...
    async function loadPeriods() {
      ...
      try {
        ...
        const json = await res.json().catch(() => null);
        const items: GradePeriod[] = Array.isArray(json?.items) ? json.items : [];
        if (cancelled) return;
        setPeriods(items);
  ...
  async function loadMatrix() {
    ...
    try {
      ...
      const periodResults: Array<{ period: GradePeriod; items: BulletinItem[]; ranks: Map<string, number> }> = [];
      ...
      for (const period of matrixPeriods) {
        try {
          ...
          const ranks = buildRankMap(
            items.map((it) => ({ student_id: it.student_id, avg: cleanAvg(it.general_avg) }))
          );
          ...
          for (const it of items) {
            ...
            row.periods[period.id] = {
              avg: cleanAvg(it.general_avg),
              rank: ranks.get(it.student_id) ?? null,
            };
          }
          ...
          setPeriodStates((prev) =>
            prev.map((s) =>
              s.period_id === period.id
                ? { ...s, status: items.length ? "ok" : "empty", message: `${items.length} élève(s)` }
                : s
            )
          );
        } catch (e: any) {
      ...
      const annualRanks = buildRankMap(
        rows.map((r) => ({ student_id: r.student_id, avg: r.annual_avg }))
      );
      rows.forEach((r) => {
  ...
  function exportCsv() {
    ...
    matrixRows.forEach((row, idx) => {
      const cells: Array<string | number | null> = [idx + 1, row.matricule || "", row.full_name];
      for (const p of loadedPeriods) {
        const cell = row.periods[p.id] || { avg: null, rank: null };
        cells.push(cell.avg !== null ? cell.avg.toFixed(2) : "", cell.rank ?? "");
      }
      cells.push(row.annual_avg !== null ? row.annual_avg.toFixed(2) : "", row.annual_rank ?? "");
  ...
  function exportPdf() {
    ...
    const periodHeader = loadedPeriods
      .map(
        (p) => `
          <th colspan="2">${escapeHtml(periodLabel(p))}<br/><span>${escapeHtml(formatDateFR(p.start_date))} — ${escapeHtml(formatDateFR(p.end_date))}</span></th>`
      )
      .join("");
    ...
    const body = matrixRows
      .map((row, idx) => {
        const periodCells = loadedPeriods
          .map((p) => {
            const cell = row.periods[p.id] || { avg: null, rank: null };
            return `<td class="num">${formatNumber(cell.avg)}</td><td class="num">${formatRank(cell.rank)}</td>`;
          })
          .join("");
...
<body>
  ...
  <div class="meta">
    ...
    <div><strong>Moyenne classe :</strong> ${formatNumber(stats.classAvg)}</div>
  ...
  <div class="footer">Document généré depuis Mon Cahier — Nexa Digital SARL</div>
    ...
    if (!win) {
      setErrorMsg("Impossible d’ouvrir la fenêtre d’impression. Vérifiez le blocage des popups.");
      return;
    }
    win.document.open();
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-200">
              Sélectionnez une classe et une année scolaire pour afficher les moyennes et rangs par période, puis la moyenne annuelle avec le rang annuel.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center text-xs text-slate-200 sm:min-w-[420px]">
            <div className="rounded-2xl border border-white/10 bg-white/8 p-3">
              <div className="text-[10px] uppercase tracking-wide text-slate-400">Élèves</div>
              <div className="mt-1 text-xl font-bold text-white">{stats.count}</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/8 p-3">
              <div className="text-[10px] uppercase tracking-wide text-slate-400">Moy. classe</div>
              <div className="mt-1 text-xl font-bold text-white">{formatNumber(stats.classAvg)}</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/8 p-3">
              <div className="text-[10px] uppercase tracking-wide text-slate-400">Périodes</div>
              <div className="mt-1 text-xl font-bold text-white">{loadedPeriods.length || matrixPeriods.length}</div>
            </div>
          </div>
        </div>
      </header>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
            ...
            >
              ...
              {classes.map((c) => (
                <option key={c.id} value={c.id}>
                  {clsLabel(c)}{c.level ? ` • ${c.level}` : ""}{c.academic_year ? ` • ${c.academic_year}` : ""}
                </option>
              ))}
            </Select>
          </div>
          ...
          <div className="flex flex-wrap gap-2 lg:col-span-4 lg:justify-end">
            <Button onClick={loadMatrix} disabled={!selectedClassId || loadingMatrix}>
              {loadingMatrix ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs">
          {matrixPeriods.length ? (
            matrixPeriods.map((p) => (
              <span key={p.id} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-slate-600">
                {periodLabel(p)} : {formatDateFR(p.start_date)} → {formatDateFR(p.end_date)}
              </span>
            ))
          ) : (
        ...
        <div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              {selectedClass ? clsLabel(selectedClass) : "Aucune classe sélectionnée"} • {selectedAcademicYear || "année courante"}
            </p>
          </div>
          ...
          <table className="min-w-full border-separate border-spacing-0 text-sm">
            <thead>
              <tr className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                ...
                {loadedPeriods.map((p) => (
                  <th key={p.id} colSpan={2} className="border-b border-r border-slate-200 bg-indigo-50 px-3 py-3 text-center text-indigo-800">
            ...
            <tbody>
              {matrixRows.length === 0 ? (
                <tr>
                  <td colSpan={3 + loadedPeriods.length * 2 + 2} className="px-6 py-14 text-center text-sm text-slate-500">
                    Chargez une classe pour afficher la matrice annuelle.
                  </td>
                </tr>
              ) : (
                matrixRows.map((row, idx) => (
                  <tr key={row.student_id} className="group odd:bg-white even:bg-slate-50/70 hover:bg-emerald-50/50">
                    ...
                    <td className="sticky left-12 z-10 border-b border-r border-slate-100 bg-inherit px-3 py-2 text-slate-600">{row.matricule || "—"}</td>
                    <td className="sticky left-44 z-10 min-w-[260px] border-b border-r border-slate-100 bg-inherit px-3 py-2 font-semibold text-slate-900">{row.full_name}</td>
                    {loadedPeriods.map((p) => {
                      const cell = row.periods[p.id] || { avg: null, rank: null };
                      return (
                        <React.Fragment key={`${row.student_id}-${p.id}`}>
                          <td className="border-b border-r border-slate-100 px-3 py-2 text-right tabular-nums">{formatNumber(cell.avg)}</td>
                          <td className="border-b border-r border-slate-100 px-3 py-2 text-right tabular-nums">{formatRank(cell.rank)}</td>
                        </React.Fragment>
                      );
                    })}
                    <td className="border-b border-r border-slate-100 bg-emerald-50/60 px-3 py-2 text-right font-bold tabular-nums text-emerald-900">{formatNumber(row.annual_avg)}</td>
                    <td className="border-b border-slate-100 bg-emerald-50/60 px-3 py-2 text-right font-bold tabular-nums text-emerald-900">{formatRank(row.annual_rank)}</td>
                  </tr>
                ))
              )}
            </tbody>
