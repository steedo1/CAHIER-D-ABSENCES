...
export default function SettingsPanel() {
  ...
  useEffect(() => {
    ...
    setMsg(null);
    setLink(null);
  }, [inst, insts]); // re-init quand on change d’établissement ou après reload

  const adminsOfInst = useMemo(() => admins.filter((a) => a.institution_id === inst), [admins, inst]);
  ...
  async function saveInst() {
    ...
    const r = await fetch(`/api/super/institutions/${inst}`, {
      ...
      body: JSON.stringify({
        name,
        code_unique: code,
        subscription_expires_at: expires || null,
      }),
    });
    const j = await r.json();
    setBusy(false);
    if (!r.ok) {
      setMsg(j?.error || "Échec de mise à jour");
      return;
    }
  ...
  async function genLink() {
    ...
    setMsg(null);
    setLink(null);
    const r = await fetch(`/api/super/admins/${admin}/password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    setBusy(false);
    if (!r.ok) {
      setMsg(j?.error || "Échec génération lien");
      return;
    }
    setLink(j?.action_link || null);
    setMsg("Lien de récupération généré.");
  }
  ...
  async function setTempPassword() {
    ...
    const r = await fetch(`/api/super/admins/${admin}/password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mode: "temp", password: pwd }),
    });
    const j = await r.json();
    setBusy(false);
    if (!r.ok) {
      setMsg(j?.error || "Échec changement mot de passe");
      return;
    }
    setMsg("Mot de passe temporaire défini.");
    setTemp(pwd); // on l’affiche pour l’admin
  }

  return (
    <div className="space-y-6">
      {/* Bloc édition établissement */}
      ...
      {/* Bloc réinitialisation mot de passe admin */}
      <div className="rounded-2xl border bg-white p-4">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-700">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <div className="mb-1 text-xs text-slate-500">Admin de l’établissement</div>
            <select
              disabled={!inst}
              value={admin}
          ...
          <div className="md:col-span-2 flex flex-wrap gap-2">
            ...
            >
              Générer un lien de récupération
            </button>

            <input
              placeholder="Mot de passe temporaire (sinon généré)"
              value={temp}
          ...
          {link && (
            <div className="md:col-span-2">
              <div className="mb-1 text-xs text-slate-500">Lien de récupération</div>
              <input
                value={link}
                readOnly
                className="w-full rounded-lg border px-3 py-2 text-sm"
