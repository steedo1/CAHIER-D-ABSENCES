...
// Helper pour déclencher /api/push/dispatch juste après l’insert en BDD.

export type Opts = {
  req?: Request;           // pour récupérer host/proto derrière un proxy (Vercel)
  reason?: string;         // tag log (ex: "attendance_bulk")
  timeoutMs?: number;      // 400..3000 (par défaut 1000)
  retries?: number;        // 0..3 (par défaut 2)
};

let lastFire = 0; // anti-storm global
...
export async function triggerPushDispatch(opts: Opts = {}) {
  ...
  const now = Date.now();
  if (now - lastFire < 200) return true; // petit debouncing
  lastFire = now;

  // Résoudre la base URL
  ...
  // Tentatives x-cron → bearer → apex/www x-cron → apex/www bearer
