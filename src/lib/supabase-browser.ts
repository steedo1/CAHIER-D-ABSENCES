...
// import type { Database } from "@/types/supabase"; // si tu as les types générés

// Cache global pour éviter de recréer le client à chaque HMR en dev
declare global {
  // eslint-disable-next-line no-var
  var __supabase__: SupabaseClient /* SupabaseClient<Database> si typé */ | undefined;
}
...
function assertClientEnv() {
  ...
  if (!url || !anon) {
    throw new Error(
      "Config Supabase manquante : définis NEXT_PUBLIC_SUPABASE_URL et NEXT_PUBLIC_SUPABASE_ANON_KEY (côté Vercel/ENV)."
    );
  }
...
export function getSupabaseBrowserClient(): SupabaseClient /* SupabaseClient<Database> si typé */ {
